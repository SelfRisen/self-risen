...
export class TranscriptionService extends BaseService {
  ...
  constructor() {
    ...
    if (config.OPENAI_API_KEY) {
      ...
      if (config.NODE_ENV === 'development') {
        this.logger.warn(
          'OPENAI_API_KEY not configured. Transcription will return placeholder text.',
        );
      }
    }
  }
  ...
  async transcribeAudio(
    audioInput: string | Express.Multer.File,
  ): Promise<string> {
    if (!this.openai) {
      if (config.NODE_ENV === 'development') {
        this.logger.warn(
          'OpenAI not configured. Returning placeholder transcription.',
        );
      }
    ...
    try {
      ...
      if (typeof audioInput === 'string') {
        if (config.NODE_ENV === 'development') {
          this.logger.log(
            `Starting transcription for audio URL: ${audioInput}`,
          );
        }
        ...
        if (config.NODE_ENV === 'development') {
          this.logger.log(
            `Starting transcription for audio file: ${audioInput.originalname || 'audio'}`,
          );
        }
        ...
        fileExtension = this.getFileExtensionFromContentType(
          contentType,
          audioInput.originalname,
        );
      }

      if (config.NODE_ENV === 'development') {
        this.logger.debug(
          `Processing audio file, Size: ${audioBuffer.length} bytes, Type: ${contentType}`,
        );
      }
      ...
      if (audioBuffer.length > 25 * 1024 * 1024) {
        ...
        throw new Error(
          `Audio file too large: ${audioBuffer.length} bytes (max 25MB)`,
        );
      }
      ...
      if (config.NODE_ENV === 'development') {
        this.logger.debug(
          `Created file object: ${fileName}, size: ${audioBuffer.length} bytes, type: ${contentType}`,
        );
      }
      ...
      if (config.NODE_ENV === 'development') {
        this.logger.log(
          `Transcription completed. Length: ${transcribedText.length} characters`,
        );
      }
      ...
      if (
        message.includes('file_size_exceeded') ||
        message.includes('too large')
      ) {
        throw new Error(
          `Failed to transcribe audio: File is too large. Maximum size is 25MB. Original error: ${message}`,
          {
            cause: error,
          },
        );
   ...
   */
  private getFileExtensionFromContentType(
    contentType: string,
    originalName?: string,
  ): string {
