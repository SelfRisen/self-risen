...
import { PERSONA_NAMES, PersonaName } from '../constants/persona.constants';
...
export class TextToSpeechService extends BaseService {
  ...
  convertNameToEnum(name: string): TtsVoicePreference | null {
    ...
    if (
      Object.values(TtsVoicePreference).includes(upper as TtsVoicePreference)
    ) {
  ...
  private getVoiceFromPreference(preference?: string | null): OpenAIVoice {
    ...
    if (config.NODE_ENV === 'development') {
      this.logger.warn(
        `Unknown voice preference: ${preference}, falling back to alloy`,
      );
    }
    return 'alloy';
  }
  ...
  async generateAffirmationAudio(
    ...
    try {
      if (config.NODE_ENV === 'development') {
        this.logger.log(
          `Generating TTS audio for affirmation (${affirmationText.length} chars)`,
        );
      }
      ...
      if (config.NODE_ENV === 'development') {
        this.logger.log(
          `TTS audio generated and uploaded: ${uploadResult.url}`,
        );
      }
