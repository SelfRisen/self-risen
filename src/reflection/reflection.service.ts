...
export class ReflectionService extends BaseService {
  ...
  async submitBelief(
    ...
    if (session.status !== 'PENDING') {
      return this.HandleError(
        new BadRequestException(
          'Belief has already been submitted for this session',
        ),
      );
    }
    ...
    if (audioFile) {
      try {
        transcriptionText =
          await this.transcriptionService.transcribeAudio(audioFile);
  ...
  async editAffirmation(
    ...
    if (
      !session.selectedAffirmationText ||
      session.selectedAffirmationText.trim().length === 0
    ) {
      return this.HandleError(
        new BadRequestException(
          'Cannot edit affirmation. No selected affirmation found in session.',
        ),
      );
    }
  ...
  async editBelief(firebaseId: string, sessionId: string, belief: string) {
    ...
    if (
      ...
      return this.HandleError(
        new BadRequestException(
          'Cannot edit belief. No selected affirmation found in session.',
        ),
      );
    }
  ...
  async reRecordBelief(
    ...
    const updatedSession = await this.prisma.reflectionSession.update({
      ...
      data: {
        ...
        limitingBelief:
          newStatus === 'BELIEF_CAPTURED' ? null : session.limitingBelief,
        selectedAffirmationText:
          newStatus === 'BELIEF_CAPTURED'
            ? null
            : session.selectedAffirmationText,
        selectedAffirmationAudioUrl:
          newStatus === 'BELIEF_CAPTURED'
            ? null
            : session.selectedAffirmationAudioUrl,
        userAffirmationAudioUrl:
          newStatus === 'BELIEF_CAPTURED'
            ? null
            : session.userAffirmationAudioUrl,
        status: newStatus,
  ...
  async createWave(firebaseId: string, dto: CreateWaveDto) {
    ...
    return this.Results(createdWave);
  }
  ...
  async getWave(firebaseId: string, waveId: string) {
    const user = await this.getUserByFirebaseId(firebaseId);
    if (!user) {
      return this.HandleError(new NotFoundException('User not found'));
    }
  ...
  async regenerateAffirmationVoice(
    ...
    const affirmationText =
      selectedAffirmation?.affirmationText ?? session.selectedAffirmationText;
    if (!affirmationText || affirmationText.trim().length === 0) {
      return this.HandleError(
        new BadRequestException(
          'Cannot regenerate voice. No selected affirmation found in session.',
        ),
      );
    }
    ...
    try {
      ...
      const voicePreference = dto?.voicePreference
        ? (this.textToSpeechService.convertNameToEnum(dto.voicePreference) ??
          selectedAffirmation?.ttsVoicePreference ??
          user.ttsVoicePreference)
        : (selectedAffirmation?.ttsVoicePreference ??
          user.ttsVoicePreference ??
          null);

      const selectedAffirmationAudioUrl =
        await this.textToSpeechService.generateAffirmationAudio(
          affirmationText,
          user.id,
      ...
      this.logger.log(
        `Regenerated affirmation audio for session ${sessionId} with voice: ${voicePreference ?? 'default'}${dto?.voicePreference ? ' (override)' : ''}`,
      );

      return this.Results(updatedSession);
    } catch (error) {
      this.logger.error(
        `Error regenerating affirmation voice: ${error.message}`,
        error.stack,
      );
      return this.HandleError(
        new BadRequestException(
          `Failed to regenerate affirmation voice: ${error.message}`,
        ),
      );
    }
  }
  ...
  async selectAffirmation(
    ...
    if (!affirmation) {
      return this.HandleError(
        new NotFoundException(
          'Affirmation not found or does not belong to this session',
        ),
      );
    }
    ...
    const voiceUsed: typeof user.ttsVoicePreference =
      affirmation.ttsVoicePreference ?? user.ttsVoicePreference ?? null;
    if (!audioUrl && affirmation.affirmationText) {
      try {
        ...
        this.logger.warn(
          `TTS generation failed for affirmation ${affirmationId}: ${ttsError.message}. Continuing without audio.`,
        );
      }
    }
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        ...
        await tx.affirmation.update({
          ...
          data: {
            ...
            ...(voiceUsed != null &&
              !affirmation.ttsVoicePreference && {
                ttsVoicePreference: voiceUsed,
              }),
          },
        });
      ...
      this.logger.log(
        `Selected affirmation ${affirmationId} for session ${sessionId}`,
      );
      ...
      this.logger.error(
        `Error selecting affirmation: ${error.message}`,
        error.stack,
      );
      return this.HandleError(
        new BadRequestException(
          `Failed to select affirmation: ${error.message}`,
        ),
      );
    }
  }
  ...
  async deleteAffirmation(firebaseId: string, affirmationId: string) {
    ...
    try {
      ...
      this.logger.error(
        `Error deleting affirmation: ${error.message}`,
        error.stack,
      );
      return this.HandleError(
        new BadRequestException(
