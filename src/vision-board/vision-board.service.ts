...
export class VisionBoardService extends BaseService {
  ...
  async addVision(
    ...
    if (!reflectionSessionId && !imageFile) {
      return this.HandleError(
        new BadRequestException(
          'Either reflectionSessionId or imageFile must be provided',
        ),
      );
    }
    ...
    if (reflectionSessionId) {
      ...
      if (
        foundSession.status !== ReflectionSessionStatus.AFFIRMATION_GENERATED
      ) {
        return this.HandleError(
          new BadRequestException(
            'Reflection session must be in AFFIRMATION_GENERATED status to add to vision board',
          ),
        );
      }
      ...
      if (existingVision) {
        return this.HandleError(
          new BadRequestException(
            'This reflection session is already in the vision board',
          ),
        );
      }
  ...
  async getAllVisions(
    ...
    const [items, totalCount] = await Promise.all([
      ...
      includeTotalCount
        ? this.prisma.vision.count({ where })
        : Promise.resolve(null),
    ]);
  ...
  async updateVision(
    ...
    if (!hasReflectionSession && !hasImage && !hasBackgroundSound) {
      return this.HandleError(
        new BadRequestException(
          'At least one of reflectionSessionId, imageFile, or visionSoundId must be provided',
        ),
      );
    }
    ...
    if (reflectionSessionId) {
      ...
      if (
        ...
        return this.HandleError(
          new BadRequestException(
            'Reflection session must be in AFFIRMATION_GENERATED status to link to vision board',
          ),
        );
      }
      if (existingItem && existingItem.id !== visionId) {
        return this.HandleError(
          new BadRequestException(
            'This reflection session is already linked to another vision',
          ),
        );
      }
    }
    ...
    let sound: Awaited<
      ReturnType<typeof this.prisma.visionSound.findUnique>
    > | null;
    try {
    ...
    if (reflectionSessionId !== undefined) {
      ...
      if (
        visionItem.reflectionSessionId &&
        visionItem.reflectionSessionId !== reflectionSessionId
      ) {
        await this.prisma.reflectionSession.update({
  ...
  async removeVision(firebaseId: string, visionId: string) {
    ...
    if (visionItem.imageUrl) {
      try {
        ...
        this.logger.warn(
          `Failed to delete image for vision ${visionId}: ${visionItem.imageUrl}`,
          error,
        );
      }
    }
  ...
  async uploadSound(firebaseId: string, soundFiles: Express.Multer.File[]) {
    ...
    if (!soundFiles || soundFiles.length === 0) {
      return this.HandleError(
        new BadRequestException('At least one audio file is required'),
      );
    }
   ...
   */
  async setVisionBackgroundSoundByName(
    firebaseId: string,
    visionId: string,
    name: string,
  ) {
  ...
  private mapVisionToResponse(
    vision: VisionWithReflectionSession,
  ): VisionResponse {
