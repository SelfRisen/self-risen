...
export class VisionBoardController extends BaseController {
  ...
  @ApiOperation({
    ...
    description:
      'Adds a vision to the vision board. Can include a reflection session (must be in AFFIRMATION_GENERATED status) and/or an image. Both are optional.',
  })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        visionBoardId: {
          type: 'string',
          description:
            'The ID of the vision board to add the vision to (required)',
  ...
  @ApiResponse({
    status: 400,
    description:
      'Invalid request or reflection session not in AFFIRMATION_GENERATED status (if provided)',
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiOperation({
    ...
    description:
      "Retrieves a paginated list of all visions in the user's vision board. Can be filtered by visionBoardId.",
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiQuery({
    ...
    type: String,
    description:
      "Filter visions by vision board ID (optional - if not provided, returns visions from all user's vision boards)",
    example: 'vision-board-id-123',
  })
  @ApiQuery({
    ...
    type: Boolean,
    description:
      'When false, skips the total count query for faster responses (e.g. next page). Default: true',
  })
  @ApiResponse({
    status: 200,
  ...
  async getVisionById(
    ...
    const result = await this.visionBoardService.getVisionById(
      user.uid,
      visionId,
    );
    if (result.isError) throw result.error;
  ...
  @ApiOperation({
    ...
    description:
      'Updates a vision in the vision board. Can update the image and/or link a reflection session. Both are optional.',
  })
  @ApiParam({
    name: 'id',
    description: 'The unique identifier of the vision',
    ...
    schema: {
      ...
      properties: {
        reflectionSessionId: {
          type: 'string',
          description:
            'The ID of the reflection session to link to the vision (optional)',
          example: 'reflection-session-id-123',
        },
        visionSoundId: {
          type: 'string',
          nullable: true,
          description:
            'ID of the vision sound from the catalog. Pass null to clear (optional)',
  ...
  @ApiResponse({
    status: 400,
    description:
      'Invalid request, failed to upload image, or reflection session validation failed',
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiOperation({
    ...
    description:
      "Selects a sound from the stater-videos music list by name and sets it as the vision's background sound. Creates a catalog entry if needed.",
  })
  @ApiParam({
    name: 'id',
    description: 'The unique identifier of the vision',
  ...
  @ApiResponse({
    status: 200,
    description: 'Vision updated with background sound',
  })
  @ApiResponse({
    status: 404,
    description: 'Vision not found or sound name not found',
  })
  async setVisionBackgroundSound(
    ...
    const result = await this.visionBoardService.setVisionBackgroundSoundByName(
      user.uid,
      visionId,
      dto.name,
    );
  ...
  async removeVision(
    ...
    const result = await this.visionBoardService.removeVision(
      user.uid,
      visionId,
    );
    if (result.isError) throw result.error;
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves all sounds (catalog used for both visions and reflections), ordered by display order.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      "Returns all sounds plus the vision's selected background sound (for picker + current selection).",
  })
  @ApiParam({ name: 'id', description: 'Vision ID', example: 'vision-id-123' })
  @ApiResponse({
    status: 200,
    description: 'Sounds and vision background sound',
  })
  ...
  async getSoundsForVision(
    ...
    const result = await this.visionBoardService.getSoundsForVision(
      user.uid,
      visionId,
    );
    if (result.isError) throw result.error;
  ...
  @ApiOperation({
    ...
    description:
      'Changes the order of a vision within its vision board. Other visions are shifted accordingly.',
  })
  @ApiParam({
    name: 'id',
  ...
  @ApiOperation({
    ...
    description:
      'Changes the order of a sound. Other sounds are shifted accordingly.',
  })
  @ApiParam({
    name: 'id',
  ...
  async createVisionBoard(
    ...
    const result = await this.visionBoardService.createVisionBoard(
      user.uid,
      dto,
    );
    if (result.isError) throw result.error;
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves all vision boards for the authenticated user, including associated category information and vision counts.',
  })
  @ApiResponse({
    status: 200,
        ...
        boards: {
          ...
          items: {
            ...
            properties: {
              ...
              visionCount: {
                type: 'number',
                description: 'Number of visions in this board',
              },
              createdAt: { type: 'string', format: 'date-time' },
              updatedAt: { type: 'string', format: 'date-time' },
            },
