...
export class AffirmationLoopController extends BaseController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Affirmation loop updated successfully',
    type: AffirmationLoopResponseDto,
  })
  @ApiResponse({
    status: 404,
    description: 'User or affirmation loop not found',
  })
  async updateLoop(
    @FirebaseUser() user: auth.DecodedIdToken,
    @Param('id') id: string,
    @Body() dto: UpdateAffirmationLoopDto,
  ) {
    const result = await this.affirmationLoopService.updateLoop(
      user.uid,
      id,
      dto,
    );
  ...
  @ApiResponse({
    ...
    type: LoopReminderResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: 'Neither morningTime nor eveningTime provided',
  })
  @ApiResponse({
    status: 404,
    description: 'User or affirmation loop not found',
  ...
  async setReminder(
    ...
    const result = await this.affirmationLoopService.setReminder(
      user.uid,
      id,
      dto,
    );
  ...
  @ApiResponse({
    status: 404,
    description: 'User, affirmation loop, or reminder not found',
  })
  async updateReminder(
    ...
    const result = await this.affirmationLoopService.updateReminder(
      user.uid,
      id,
      dto,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Deactivates the reminder schedule for this loop without deleting it.',
  })
  @ApiParam({ name: 'id', description: 'Affirmation loop ID' })
  @ApiResponse({
    status: 200,
  ...
  async deleteReminder(
    ...
    const result = await this.affirmationLoopService.disableReminder(
      user.uid,
      id,
    );
    if (result.isError) throw result.error;
