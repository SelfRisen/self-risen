...
export class AffirmationLoopService extends BaseService {
  ...
  constructor(
    ...
    @InjectQueue('audio_merge')
    private readonly audioMergeQueue: Queue<AudioMergeJobData>,
  ) {
    super();
  }
  ...
  async createLoop(firebaseId: string, dto: CreateAffirmationLoopDto) {
    ...
    if (dto.affirmationIds.length > this.MAX_AFFIRMATIONS) {
      return this.HandleError(
        new BadRequestException(
          `Maximum ${this.MAX_AFFIRMATIONS} affirmations per loop`,
        ),
      );
    }
    ...
    if (!this.staterVideosService.getSoundByName(dto.backgroundMusicKey)) {
      return this.HandleError(
        new BadRequestException(
          `Unknown background music: ${dto.backgroundMusicKey}`,
        ),
      );
    }
  ...
  async updateLoop(
    ...
    if (!loop) {
      return this.HandleError(
        new NotFoundException('Affirmation loop not found'),
      );
    }
  ...
  async setReminder(
    ...
    if (!dto.morningTime && !dto.eveningTime) {
      return this.HandleError(
        new BadRequestException(
          'At least one of morningTime or eveningTime is required',
        ),
      );
    }
  ...
  async updateReminder(
    ...
    const existing = await this.prisma.loopReminder.findUnique({
      where: { loopId },
    });
    if (!existing) {
      return this.HandleError(
        new NotFoundException('Reminder not found for this loop'),
      );
    }
  ...
  async deleteLoop(firebaseId: string, loopId: string) {
    ...
    if (loop.audioPath) {
      try {
        ...
        this.logger.warn(
          `Failed to delete loop audio ${loop.audioPath}: ${
